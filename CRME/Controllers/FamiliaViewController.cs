...
{
    public class FamiliaViewController : Controller
    {
        // GET: FamiliaView
        ...
        public ActionResult SaveCondicion(Familia condicion)
        {
            //Empresa empresas = new Empresa();
            var serializerCat = new JavaScriptSerializer();
            bool success = false;
            string mensajefound = "";
            var found = db.Familia.FirstOrDefault(x => x.Descripcion == condicion.Descripcion);
            ...
            {
                if (condicion.Id_Familia == 0)
                {
                    try
                    {
                        Familia condi = new Familia();
                        condi.Descripcion = condicion.Descripcion;
                        db.Familia.Add(condi);
                        if (db.SaveChanges() > 0)
                        {
                            success = true;
                        }
                ...
                else
                {
                    try
                    {
                        Familia Condi = db.Familia.Find(condicion.Id_Familia);
                        Condi.Descripcion = condicion.Descripcion;
        ...
        public ActionResult _FormularioFamilia(long? Id_Condicion)
        {  //codigo para agregar y editar usuarios
            //UsuariosPersonas Personas = new UsuariosPersonas();
            Familia condicion = new Familia();
            // Personas Persona = new Personas();

            if (Id_Condicion != null)
            {
                ViewBag.edit = 1;
                condicion = db.Familia.Find(Id_Condicion);
            }


            return PartialView(condicion);
        }
        public ActionResult _TablaFamilia(int? page)
        {
        ...
        public ActionResult DeleteUsuario(long? Em_Cve_Empresa)
        {
            ...
            try
            {
                Familia condi = db.Familia.Find(Em_Cve_Empresa);
