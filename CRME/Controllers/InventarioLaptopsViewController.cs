...
{
    public class InventarioLaptopsViewController : Controller
    {
        // GET: EmpresasView
        ...
        public ActionResult SaveEmpresa(inventario_laptop Empresas)
        {
            ...
            var found = db.inventario_laptop.FirstOrDefault(x => x.serie == Empresas.serie && x.inv_laptop_ID != Empresas.inv_laptop_ID);

            if (string.IsNullOrWhiteSpace(Empresas.serie))
            {
                mensajefound = "¡No puede estar en blanco el Numero de Serie de la laptop!";
            }
            else
            {
                ...
                {
                    if (Empresas.inv_laptop_ID == 0)
                    {
                        try
                        {
                            inventario_laptop empre = new inventario_laptop();
                            ...
                            inventario_laptop modificador = db.inventario_laptop.OrderByDescending(x => x.inv_laptop_ID).First();
                            ...
                            empre.cod_inventario = etiqueta.Em_Descripcion.ToUpper() + maspruba[8] + maspruba[9] + "-LP" + Convert.ToString(modificador.inv_laptop_ID + 1);

                            db.inventario_laptop.Add(empre);
                            if (db.SaveChanges() > 0)
                            {
                                success = true;
                            }
                    ...
                    else
                    {
                        try
                        {
                            inventario_laptop Empre = db.inventario_laptop.Find(Empresas.inv_laptop_ID);
                            Empre.serie = Empresas.serie;
                            Empre.marca = Empresas.marca;
                            Empre.modelo = Empresas.modelo;
                            ...
                            Empre.cod_inventario = temporal.ToUpper() + maspruba[8] + maspruba[9] + "-LP" + Empresas.inv_laptop_ID;
        ...
        {  //codigo para agregar y editar usuarios
            ...
            inventario_laptop Empresas = new inventario_laptop();
           // Personas Persona = new Personas();

            if (inv_laptop_ID != null)
            {
                ViewBag.edit = 1;
                Empresas = db.inventario_laptop.Find(inv_laptop_ID);
        ...
        public ActionResult _TablaLaptops(int? page, string filtro)
        {
            ...
            if (string.IsNullOrEmpty(filtro))
            {
                ...
                var lista = db.inventario_laptop.OrderByDescending(x=> x.inv_laptop_ID).ToList();
                ViewBag.filtro = filtro;
                return PartialView(lista.ToPagedList(pageNumber, pageSize));
            }
            else
            {
                var lista = db.inventario_laptop.Where(x => x.serie.ToUpper().Contains(filtro.ToUpper().Trim())
        ...
        public ActionResult ExportarExcel(int? creado, string filtro)
        {
            ...
            if (creado == 1)
            {
                ...
                var ruta = "~/Upload/Temporales/Descargas/" + "Inventario Laptops - " + DateTime.Now.ToShortDateString().Replace("/", "-") + ".xlsx";
                return File(Url.Content(ruta), excelContentType, "Inventario Laptops - " + DateTime.Now.ToShortDateString().Replace("/", "-") + ".xlsx");
            }
            // Aqui se debe cambiar por "Resguardos Laptops"
            if (System.IO.File.Exists(System.Web.Hosting.HostingEnvironment.MapPath("~/Upload/Temporales/Descargas/" + "Inventario Laptops - " + DateTime.Now.ToShortDateString().Replace("/", "-") + ".xlsx")))
            {
                System.IO.File.Delete(System.Web.Hosting.HostingEnvironment.MapPath("~/Upload/Temporales/Descargas/" + "Inventario Laptops - " + DateTime.Now.ToShortDateString().Replace("/", "-") + ".xlsx"));
            }

            string savedFileName = Path.Combine(System.Web.Hosting.HostingEnvironment.MapPath("~/Upload/Temporales/Descargas/" + "Inventario Laptops - " + DateTime.Now.ToShortDateString().Replace("/", "-") + ".xlsx"));
            ...
            using (FileStream stream = new FileStream(savedFileName, FileMode.Create))
            {
                try
                {
                    ...
                    var productos = db.Database.SqlQuery<Inventario_Lista_Laptops_Excel>("Sp_Get_Inventario_Laptops_excel @filtro", new SqlParameter("@filtro", filtro)).ToList();

                    using (var libro = new ExcelPackage())
                    {

                        var worksheet = libro.Workbook.Worksheets.Add("Inventario Laptops");
                        ...
                        var titlers = cellrs.RichText.Add("Inventario de Laptops");
                        titlers.Bold = true;
                        ...
                        var tabla = worksheet.Tables.Add(new ExcelAddressBase(fromRow: 6, fromCol: 2, toRow: productos.Count + 6, toColumn: 11), "inventario");
