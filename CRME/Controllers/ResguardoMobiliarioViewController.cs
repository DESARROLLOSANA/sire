...
{
    public class ResguardosMobiliarioViewController : Controller
    {
        // GET: ResguardosView
        ...
        public ActionResult ExportarExcel(int? creado)
        {
            ...
            if (creado == 1)
            {
                var ruta = "~/Upload/Temporales/Descargas/" + "Resguardos mobilarios - " + DateTime.Now.ToShortDateString().Replace("/", "-") + ".xlsx";
                return File(Url.Content(ruta), excelContentType, "Resguardos mobilarios - " + DateTime.Now.ToShortDateString().Replace("/", "-") + ".xlsx");
                //return File(Url.Content(ruta), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Egresados.xlsx");
            }
            if (System.IO.File.Exists(System.Web.Hosting.HostingEnvironment.MapPath("~/Upload/Temporales/Descargas/" + "Resguardos mobilarios - " + DateTime.Now.ToShortDateString().Replace("/", "-") + ".xlsx")))
            {
                System.IO.File.Delete(System.Web.Hosting.HostingEnvironment.MapPath("~/Upload/Temporales/Descargas/" + "Resguardos mobilarios - " + DateTime.Now.ToShortDateString().Replace("/", "-") + ".xlsx"));
            }

            string savedFileName = Path.Combine(System.Web.Hosting.HostingEnvironment.MapPath("~/Upload/Temporales/Descargas/" + "Resguardos mobilarios - " + DateTime.Now.ToShortDateString().Replace("/", "-") + ".xlsx"));
            ...
            using (FileStream stream = new FileStream(savedFileName, FileMode.Create))
            {
                try
                {
                    ...
                    using (var libro = new ExcelPackage())
                    {
                        ...
                        var worksheet = libro.Workbook.Worksheets.Add("Resguardos mobilarios");
                        ...
                        var titlers = cellrs.RichText.Add("Resguardos de mobilarios");
                        titlers.Bold = true;
