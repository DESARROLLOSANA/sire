        ...
        public ActionResult ExportarExcel(int? creado, string filtro)
        {
            ...
            using (FileStream stream = new FileStream(savedFileName, FileMode.Create))
            {
                try
                {
                    ...
                    List<Resguardos_Lista_Moviles_Excel> productos = new List<Resguardos_Lista_Moviles_Excel>();
                    if (filtro == null || filtro == "")
                    {
                        productos = db.Database.SqlQuery<Resguardos_Lista_Moviles_Excel>("Sp_Get_Resguardos_Moviles_excel").ToList();
                    }
                    else
                    {
                        productos = db.Database.SqlQuery<Resguardos_Lista_Moviles_Excel>("Sp_Get_Resguardos_Moviles_excel_filtro @filtro", new SqlParameter("@filtro", filtro)).ToList();
                    }
