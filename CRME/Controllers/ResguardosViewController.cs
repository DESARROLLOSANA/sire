        ...
        public ActionResult ExportarExcel(int? creado, string filtro)
        {
            ...
            using (FileStream stream = new FileStream(savedFileName, FileMode.Create))
            {
                try
                {
                    ...
                    List<resguardos_lista_laptos_excel> productos = new List<resguardos_lista_laptos_excel>();
                    if (filtro == null || filtro == "")
                    {
                        productos = db.Database.SqlQuery<resguardos_lista_laptos_excel>("Sp_Get_Resguardos_Latop_excel").ToList();
                    }
                    else
                    {
                        productos = db.Database.SqlQuery<resguardos_lista_laptos_excel>("Sp_Get_Resguardos_Latop_excel_filtro @filtro", new SqlParameter("@filtro", filtro)).ToList();
                    }
