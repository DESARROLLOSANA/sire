...
{
    public class SubfamiliaViewController : Controller
    {
        // GET: SubfamiliaView
        ...
        public ActionResult SaveCondicion(Subfamilia subfam)
        {
            //Empresa empresas = new Empresa();
            var serializerCat = new JavaScriptSerializer();
            bool success = false;
            string mensajefound = "";
            var found = db.Subfamilia.FirstOrDefault(x => x.Descripcion == subfam.Descripcion);
            ...
            {
                if (subfam.Id_Sub_fam == 0)
                {
                    try
                    {
                        Subfamilia subf = new Subfamilia();
                        subf.Id_Familia = subfam.Id_Familia;
                        subf.Descripcion = subfam.Descripcion;
                        db.Subfamilia.Add(subf);
                        if (db.SaveChanges() > 0)
                        {
                            success = true;
                        }
                ...
                else
                {
                    try
                    {
                        Subfamilia Condi = db.Subfamilia.Find(subfam.Id_Sub_fam);
                        Condi.Id_Familia = subfam.Id_Familia;
                        Condi.Descripcion = subfam.Descripcion;                      
        ...
        public ActionResult _FormularioSubfamilia(long? Id_Condicion)
        {  //codigo para agregar y editar usuarios
            //UsuariosPersonas Personas = new UsuariosPersonas();
            Subfamilia condicion = new Subfamilia();
            // Personas Persona = new Personas();
            if (Id_Condicion != null)
            {
                ViewBag.edit = 1;
                condicion = db.Subfamilia.Find(Id_Condicion);
                ViewBag.Familia = new SelectList(db.Familia.ToList(), "Id_Familia", "Descripcion",  condicion.Id_Familia);
            }
            else
            {
        ...
        public ActionResult _TablaSubfamilia(int? page)
        {
            const int pageSize = 10;
            int pageNumber = (page ?? 1);

            var lista = db.Subfamilia.ToList();
        ...
        public ActionResult DeleteUsuario(long? Em_Cve_Empresa)
        {
            ...
            try
            {
                Subfamilia condi = db.Subfamilia.Find(Em_Cve_Empresa);
