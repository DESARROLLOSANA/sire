...
{
    public class UnidadMedidaViewController : Controller
    {
        // GET: MedidasView
        ...
        public ActionResult SaveCondicion(Unidad_Medida condicion)
        {
            //Empresa empresas = new Empresa();
            var serializerCat = new JavaScriptSerializer();
            bool success = false;
            string mensajefound = "";
            var found = db.Medida.FirstOrDefault(x => x.Descripcion == condicion.Descripcion);

            if (found != null)
            {
                mensajefound = "¡Ya existe una medida que coincide con el ingresado!";

            }
            else
            {
                if (condicion.Id_UM == 0)
                {
                    try
                    {
                        Unidad_Medida condi = new Unidad_Medida();
                        condi.Descripcion = condicion.Descripcion;
                        condi.Abreviacion = condicion.Abreviacion;
                        db.Unidad_Medida.Add(condi);
                        if (db.SaveChanges() > 0)
                        {
                            success = true;
                        }
                ...
                else
                {
                    try
                    {
                        Unidad_Medida Condi = db.Unidad_Medida.Find(condicion.Id_UM);
                        Condi.Descripcion = condicion.Descripcion;
                        Condi.Abreviacion = condicion.Abreviacion;
        ...
        public ActionResult _FormularioMedidas(long? Id_Condicion)
        {  //codigo para agregar y editar usuarios
            //UsuariosPersonas Personas = new UsuariosPersonas();
            Unidad_Medida condicion = new Unidad_Medida();
            // Personas Persona = new Personas();

            if (Id_Condicion != null)
            {
                ViewBag.edit = 1;
                condicion = db.Unidad_Medida.Find(Id_Condicion);
            }


            return PartialView(condicion);
        }
        public ActionResult _TablaMedidas(int? page)
        {
        ...
        public ActionResult DeleteUsuario(long? Em_Cve_Empresa)
        {
            ...
            try
            {
                Unidad_Medida condi = db.Unidad_Medida.Find(Em_Cve_Empresa);
